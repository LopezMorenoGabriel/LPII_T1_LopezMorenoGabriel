...
import model.DentistaGabrielLM;
import model.EquipoGabrielLM;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
		...
		try {
			String jpql = "select d from DentistaGabrielLM d";
			List<DentistaGabrielLM> lstDentistas = manager.createQuery(jpql, DentistaGabrielLM.class).getResultList();

			for (DentistaGabrielLM dentista : lstDentistas) {
	...
	void listar() {
		...
		String jpql = "select e,d.email,es.desTitulo,d.cop from EquipoGabrielLM e join e.dentista d join d.especialidad es order by e.nroEquipo";

		try {
			List<Object[]> resultados = manager.createQuery(jpql, Object[].class).getResultList();

			for (Object[] objects : resultados) {
				EquipoGabrielLM equipo = (EquipoGabrielLM) objects[0];
				String emailDentista = (String) objects[1];
				String nomEspecialidad = (String) objects[2];
				int cop = (int) objects[3];
				imprimir("Nro de equipo.............: " + equipo.getNroEquipo());
				imprimir("Nombre....................: " + equipo.getNomEquipo());
				imprimir("Costo.....................: " + equipo.getPrecio());
				imprimir("Fecha de adquisición......: " + equipo.getFchAdquisicion());
				imprimir("Dentista..................: " + cop + " - " + equipo.getDentista());
				imprimir("Correo....................: " + emailDentista);
				imprimir("Especialidad..............: " + nomEspecialidad);
	...
	void adicionar() {
		...
		DentistaGabrielLM dentista = (DentistaGabrielLM) cboDentistas.getSelectedItem();
		String estado = (String) cboEstados.getSelectedItem();

		EquipoGabrielLM equipo = new EquipoGabrielLM(nombre, costo, dentista,estado);
	...
	void buscar() {
		...
		try {
			EquipoGabrielLM equipo = manager.find(EquipoGabrielLM.class, idEquipo);
			...
			txtNombre.setText(equipo.getNomEquipo());
			txtCosto.setText(String.valueOf(equipo.getPrecio()));
			cboEstados.setSelectedItem(equipo.getEstado());
			txtFechaAdquisicion.setText(equipo.getFchAdquisicion() + "");
	...
	void modificar() {
		...
		EquipoGabrielLM eq = new EquipoGabrielLM();;
		eq.setNroEquipo(idEquipo);
		eq.setDentista((DentistaGabrielLM) cboDentistas.getSelectedItem());
		eq.setEstado((String) cboEstados.getSelectedItem());
		eq.setFchAdquisicion(LocalDate.now());
		...
		try {
			...
			mensajeInfo("Solicitud actulizada");
		} catch (Exception e) {
	...
	void eliminar() {
		...
		EntityManager manager = JPAUtil.getEntityManager();
		try {
			EquipoGabrielLM eq = manager.find(EquipoGabrielLM.class, id);

			if (eq == null) {
				mensajeInfo("No se encontro el equipo");
				return;
			}
